    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-12">
      <div className="flex items-center justify-between">
        ...
        <form action="/api/admin/logout" method="POST">
          <button className="rounded bg-stone-900 px-4 py-2 text-sm font-semibold text-white">Logout</button>
        </form>
      </div>
              ...
              <tr key={slug} className="border-t border-stone-200">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <Link href={`/demo/${slug}`} className="rounded bg-stone-900 px-3 py-1.5 text-xs font-semibold text-white">View Demo</Link>
                    <Link href={`/admin?store=${slug}`} className="rounded bg-stone-200 px-3 py-1.5 text-xs font-semibold text-stone-800">Edit Data</Link>
                  </div>
                </td>
              </tr>
            ))}
